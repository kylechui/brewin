...
ValueType: TypeAlias = Optional["bool | int | str | ObjectDef"]


# Enumerated type for our different language data types
class Type:
    def __init__(self, type_name: str, supertype_name: Optional[str] = None) -> None:
...
class Value:
    def __init__(self, type_obj: Type, value: ValueType = None):
        self.t = type_obj
        self.v = value
    ...
    def is_typeless_null(self) -> bool:
...
def create_value(val: str) -> Optional[Value]:
...
def create_default_value(type_def: Type) -> Value:
...
class TypeManager:
    def __init__(self) -> None:
        self.map_typename_to_type: dict[str, Type] = {}
    ...
    def add_class_type(self, class_name: str, superclass_name: Optional[str]) -> None:
        class_type: Type = Type(class_name, superclass_name)
    ...
    def get_type_info(self, typename: Optional[str]) -> Optional[Type]:
        if typename is None or not self.is_valid_type(typename):
            return None
    ...
    def is_a_subtype(self, suspected_supertype: str, suspected_subtype: str) -> bool:
        ...
        while True:
            ...
            if type_info is None or type_info.supertype_name is None:
                return False
            cur_type = (
    ...
    def check_type_compatibility(
        self, typea: Type, typeb: Type, for_assignment: bool
    ) -> bool:
    ...
    def __setup_primitive_types(self) -> None:
